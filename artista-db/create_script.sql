...
BEGIN
	...
	UPDATE `picture`
    SET isProfile = 0 
    WHERE `seller` = seller AND `id` != newId;    
END$$
...
BEGIN
	DECLARE numRows INT;
	SELECT COUNT(*) INTO numRows FROM `listing_picture` WHERE `listing_id` = listing;
    
    INSERT INTO `picture`(`seller`,`isProfile`,`path`) VALUES (seller,0,path);
    
    IF numRows = 0
    THEN UPDATE `listing` SET `mainPic` = LAST_INSERT_ID() WHERE `id` = listing;
    END IF;
    
    INSERT INTO `listing_picture`(`listing_id`,`picture_id`) VALUES (listing, LAST_INSERT_ID());
END$$
